...
import 'package:getx_demo1/app/widgets/scrollable_gesture_processing.dart';
...
class ListViewEmbedListViewPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ScrollableGestureProcessingWidget<ScrollController, ScrollController>(
        builder: (pageController, listController, phsyics) {
          return ListView(
            physics: phsyics,
            controller: pageController,
...
class _EmabedPageViewState extends State<EmabedPageView> with TickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    final child = ScrollableGestureProcessingWidget<PageController, PageController>(
                    ...
                    if (e == '风景') {
                      ...
                      return KeepAliveWrapper(
                        child: Column(
                          children: [
                            TabBar(
                              tabs: secoundsTabs.map((e) => Tab(text: e)).toList(),
